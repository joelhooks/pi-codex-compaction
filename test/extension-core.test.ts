...
test("chooses queued seam before preturn budget", () => {
  ...
  assert.equal(defaultTuning.queuedSeamTokens, 120_000);
  assert.equal(defaultTuning.preturnBudgetTokens, 150_000);
...
test("uses absolute token limits when model context metadata is too optimistic", () => {
  assert.equal(
    chooseCompactionReason({ tokens: 121_000, percent: 30, hasPendingMessages: true }),
    "queued_seam",
  );
  assert.equal(
    chooseCompactionReason({ tokens: 151_000, percent: 38, hasPendingMessages: false }),
    "preturn_budget",
  );
  assert.equal(
    chooseCompactionReason({ tokens: 119_999, percent: 29, hasPendingMessages: true }),
    undefined,
  );
});
